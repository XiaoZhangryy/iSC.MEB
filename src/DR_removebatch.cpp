// This script implement integrating any finite data samples by 
// separating spatial information (f_{mi}) and cluster structure (z_{mi}) in embeddings (v_{mi})
// Date: 2022-08-31
  ...
  if(mix_prop_heter){
    for(r=0; r < M; ++r){
      ...
      for(k = 0; k < ng_beta; ++k){ // each sample has a smoothing parameter
        objBetaVec(k) =  objr_beta(yf(r), Rf(r), Adjf(r), K, alpha0, beta_grid(k)); 
      }
      beta0(r) = beta_grid(index_max(objBetaVec));
    }
  }else{
    for(k=0; k < ng_beta; ++k){ // all samples have a same smoothing parameter.
      objBetaVec(k) = obj_beta(yf, Rf, Adjf, K, alpha0, beta_grid(k));
    }
...
void runICM_sp_embed(const arma::field<mat>& Vf, arma::field<ivec>& yf, arma::field<mat>& Ff, 
  ...
  for(r = 0; r < r_max; ++r){
    ...
    for (k = 0; k < K; k++)	{
      ...
      multi_det_Sk_embedCpp(Vf(r) - Muf(r), SrkI, Mu0.row(k), // Use SVD to speed up.
                      logdSk, mSk);
  ...
  for(r= 0; r< r_max; r++){
    ...
    for (iter = 1; iter < maxIter_ICM;  ++iter ) {
      ...
      Uy1 = calYenergy2D_sp(yf(r), Adjf(r), K, alpha0, beta0(r));
      //Rprintf("predict Y and V,  iter = %d ! \n", iter);
...
// [[Rcpp::export]]
Rcpp::List SepSpatialClusterCpp(
